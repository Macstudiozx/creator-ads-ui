  ...
  return <>
    ...
    <div className="stage">
      ...
      <section className="ops-v2">
        <div className="ops-v2-head"><div><span className="eyebrow">Original UI v2</span><h3>Launch readiness flow</h3><p>แยกสถานะ Campaign / Ad Set / Creative / Ad ให้ชัด ก่อนสร้างต่อหรือเปิดยิง</p></div><span className={`ops-live ${createdAds.length ? 'ready' : skippedAds.length ? 'warn' : 'planned'}`}>{createdAds.length ? 'Ad exists · PAUSED' : skippedAds.length ? 'Need media completion' : 'Ready for PAUSED create'}</span></div>
